...
- **LoRA stack**: part of global image settings (`imageGeneration.loras`). Each entry is `{ name, strength, mode, on, triggerWord }` where `mode` is `on` (always applied), `off`, or `auto` (applied only when its `triggerWord` keyword appears in the prompt; the keyword doubles as the trigger word). `on` is derived (`mode === 'on'`) for legacy filters. `resolveActiveLoras`/`resolveTriggerWords` in `image-generator.js` resolve the per-prompt stack for the T2I graph; auto keywords already present in the prompt are not re-prepended, and `stripLoraTriggerWords` only strips explicit `on` trigger words so auto keywords survive. Trigger words from active LoRAs are prepended to the image prompt at generation time. Video LoRAs remain on/off only.
